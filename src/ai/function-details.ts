...
export function functionDetailsPayload(func: AppFunction) {
  ...
  if (func.functionType === "ai") {
    payload.ai_request_prompt = func.aiRequestPrompt || "";
  } else if (func.functionType === "javascript") {
    payload.code = func.code || "";
  } else {
    ...
    payload.http_request = http;
    payload.extractor_type = func.extractorType ?? "javascript";

    if (func.extractorType === "ai") {
      payload.extractor_prompt = func.extractorPrompt || "";
    } else {
      payload.extractor_code = func.extractorCode;
    }
